...
def load_and_clean(alt_embed=2):
    ...
    if alt_embed == 0 or alt_embed == 1:
        ...
        item_features = text_features.text_feature_to_vec(items.item_name.values, D=3, epochs=50, learning_rate=0.0005)
        text_features.show_neighbors(items.item_name.values, item_features)

        shop_features = text_features.text_feature_to_vec(shops.shop_name.values, D=3, epochs=500, learning_rate=0.01)
        text_features.show_neighbors(shops.shop_name.values, shop_features)

        cat_features = text_features.text_feature_to_vec(cats.item_category_name.values, D=3, epochs=500, learning_rate=0.01)
        text_features.show_neighbors(cats.item_category_name.values, cat_features)

        utils.embed_categorical_features()
        items = pd.read_csv('../data/items_nn%s.csv' % (1 if alt_embed else ''))
        shops = pd.read_csv('../data/shops_nn%s.csv' % (1 if alt_embed else ''))
        cats = pd.read_csv('../data/item_categories_nn%s.csv' % (1 if alt_embed else ''))
        if alt_embed == 1:
...
def extend_train(train, test, items, shops, cats):
    # train, test, items, shops, cats = load_and_clean(2)
    ...
    for c in ['item_0', 'item_1', 'item_2', 'item_3', 'item_4',
              ...
              'cat_0', 'cat_1', 'cat_2']:
        matrix[c] = (matrix[c] - matrix[c].mean()) / (matrix[c].std() + 1e-10)

    ts = matrix['date_block_num'].unique()
    unseen = defaultdict(dict)
    cat_names = ['item_id', ] #'item_category_id', 'shop_id']
    for t in sorted(ts):
        for cat_name in cat_names:
            _t = matrix[(matrix['date_block_num'] == t)][cat_name].unique()
            _before_t = matrix[(matrix['date_block_num'] < t)][cat_name].unique()
            unseen[cat_name][t] = np.setdiff1d(_t, _before_t)

    for t in tqdm(ts):
        mask = (matrix['date_block_num'] == t) & (matrix['item_id'].isin(unseen['item_id'][t]))
        matrix.loc[mask, ['item_0', 'item_1', 'item_2', 'item_3', 'item_4']] = [-10] * 5
