...
def load_and_clean(alt_embed=0):
    # train = pd.read_csv('../data/sales_train1.csv')
    train = pd.read_csv('../data/sales_train.csv').drop('date', axis=1)
    test = pd.read_csv('../data/test.csv')

    if alt_embed == 0 or alt_embed == 1:
        text_features.embed_categorical_features()
        items = pd.read_csv('../data/items_nn%s.csv' % (1 if alt_embed else '')).drop('item_name', axis=1)
        shops = pd.read_csv('../data/shops_nn%s.csv' % (1 if alt_embed else '')).drop('shop_name', axis=1)
        cats = pd.read_csv('../data/item_categories_nn%s.csv' % (1 if alt_embed else '')).drop('item_category_name', axis=1)
        if alt_embed == 1:
            items_ = pd.read_csv('../data/items.csv').drop('item_name', axis=1)
            items = items.merge(items_, on='item_id', how='left')
    else:
        items = pd.read_csv('../data/items_nn2.csv').drop('Unnamed: 0', axis=1)
        shops = pd.read_csv('../data/shops_nn2.csv').drop('Unnamed: 0', axis=1)
        cats = pd.read_csv('../data/item_categories_nn2.csv').drop('Unnamed: 0', axis=1)

    for c in ['item_0', 'item_1', 'item_2', 'item_3', 'item_4',]:
        items[c] = ((items[c] - items[c].min()) / (items[c].max() - items[c].min()))
        items[c] = items[c].round(3).astype(np.float32)

    for c in ['shop_0', 'shop_1', 'shop_2',]:
        shops[c] = (shops[c] - shops[c].min()) / (shops[c].max() - shops[c].min())
        shops[c] = shops[c].round(3).astype(np.float32)

    for c in ['cat_0', 'cat_1', 'cat_2']:
        cats[c] = (cats[c] - cats[c].min()) / (cats[c].max() - cats[c].min())
        cats[c] = cats[c].round(3).astype(np.float32)
...
def extend_train(train, test, items, shops, cats):
    # train, test, items, shops, cats = load_and_clean(0)
    ...
    time.time() - ts
    # text_features.show_neighbors(items.item_name.values, items[items.columns[-5:]].fillna(0).values, i=None)

    for c in matrix.columns:
        if matrix[c].dtype == 'float16' or matrix[c].dtype == 'float64':
            matrix[c] = matrix[c].astype(np.float32)

    # matrix.drop(['item_name', 'shop_name', 'item_category_name'], axis=1, inplace=True)
    matrix.to_parquet('../data_tmp/extend_train.snappy')
    return matrix


def mean_enc(matrix, train=None, group_cols=[], group_shorts=[], enc_col='item_cnt_month', agg_fn='mean', lags=[1, 2, 3, 6, 12]):
    ...
    matrix = utils.lag_feature(matrix, lags, enc_name, id_cols=group_cols)
    matrix.drop(enc_name, axis=1, inplace=True)
    return matrix


def evaluate_features(data, t_start=11, t_valid=[22], target = 'item_cnt_month',
    ...
    model_kwargs = dict(max_depth=10, n_estimators=100, min_child_weight=300,
                        colsample_bytree=0.6, subsample=0.8, eta=0.05, seed=42, tree_method='gpu_hist',)
    fs = feature_stats.FeatureStatisticsRF(X_train, Y_train, X_valid, Y_valid, **model_kwargs)
    if show_plots:
        fs.show_plots()
    return fs
...
def train_xgb(matrix=None, path=None,
              target='item_cnt_month',
              drop_cols=[],
              add_drop=['date_block_num', 'item_id'],
              max_depth=8, n_estimators=100, eta=0.2, min_child_weight=300, colsample_bytree=0.8,
              subsample=0.8,
              t_start=11,
              t_valid=[33],
    ...
    data = matrix[matrix.date_block_num > t_start]
    ...
    X_valid = data[data.date_block_num.isin(t_valid)].drop([target] + add_drop, axis=1)
    Y_valid = data[data.date_block_num.isin(t_valid)][target]
    X_test = data[data.date_block_num == 34].drop([target] + add_drop, axis=1)
    ...
    return model, X_train, Y_train, X_valid, Y_valid, X_test


def gen_submission(model, X_train, Y_train, X_valid, Y_valid, X_test):
    ...
    submission_id = len(list(iglob('../submission/xgb_submission_%s*.csv' % date)))
    submission_name = '../submission/xgb_submission_%s_%s.csv' % (date, submission_id)
    submission_valid_name = '../submission/xgb_submission_%s_%s_valid.csv' % (date, submission_id)
    submission.to_csv(submission_name, index=False)
    pd.Series(Y_pred).to_csv(submission_valid_name, index=False)

    return submission_name


def features_20200614():
    train, test, items, shops, cats = load_and_clean(alt_embed=0)
    # matrix = extend_train(train, test, items, shops, cats)
    matrix = pd.read_parquet('../data_tmp/extend_train.snappy')
    matrix.info()

    # my special feature should shine or fail here!
    ts = time.time()
    matrix = utils.lag_feature(matrix, [1, 2, 3, 6, 12], 'item_cnt_month', id_cols=['date_block_num', 'shop_id', 'item_id'])
    time.time() - ts
    ...
    for agg_cols, agg_names, lags in tqdm(
            [(['date_block_num', ],                              ['date', ],              [1]),
             ...
             (['date_block_num', 'item_category_id'],            ['date', 'cat'],         [1]),
             (['date_block_num', 'shop_id', 'item_category_id'], ['date', 'shop', 'cat'], [1]),
        ...
        matrix = utils.lag_feature(matrix, lags, mean_enc_name, id_cols=agg_cols)
        matrix.drop([mean_enc_name], axis=1, inplace=True)
        print(time.time() - ts)
    ...
    lags = [1, 2, 3, 4, 5, 6]
    matrix = utils.lag_feature(matrix, lags, 'date_item_avg_item_price', id_cols=['date_block_num', 'item_id'])
    ...
    matrix = utils.lag_feature(matrix, [1], 'delta_revenue', id_cols=['date_block_num', 'shop_id'])
    ...
    matrix.to_parquet('../data_tmp/data_2020-06-14_01_replica.snappy')
    matrix = pd.read_parquet('../data_tmp/data_2020-06-14_01_replica.snappy')


def features_20209716_0():
    ...
    # [99]	validation_0-rmse:0.88220	validation_1-rmse:1.06415

    matrix.to_parquet('../data_tmp/tmp.snappy')
    matrix = pd.read_parquet('../data_tmp/tmp.snappy')

    # mean encoding the categorical features
    target = 'item_cnt_month'
    for agg_cols, agg_names, lags in tqdm(
            [
             # (['date_block_num', 'shop_id'],  ['date', 'shop'], [1, 2, 3, 6, 12]),
             (['date_block_num', 'item_id'],  ['date', 'item'], [1, 2, 3, 6, 12]),
             (['date_block_num', 'item_category_id'],  ['date', 'cat'], [1, 2, 3, 6, 12]),
             (['date_block_num', 'item_category_id', 'shop_id'],  ['date', 'cat', 'shop'], [1, 2, 3, 6, 12]),
             ]):
        enc_name = '_'.join(agg_names + ['avg', 'item'])
        group = matrix.groupby(agg_cols).agg({target: ['mean']})
        group.columns = [enc_name]
        matrix = pd.merge(matrix, group.reset_index(), on=agg_cols, how='left')
        matrix[enc_name] = matrix[enc_name].astype(np.float32)
        matrix = utils.lag_feature(matrix, lags, enc_name, id_cols=agg_cols)
        matrix.drop(enc_name, axis=1, inplace=True)

    drop_cols = ['ID', 'shop_0', 'shop_1', 'shop_2', 'cat_0', 'cat_1', 'cat_2', 'item_0', 'item_1', 'item_2', 'item_3', 'item_4']
    fs = evaluate_features(matrix, t_start=11, t_valid=[22], drop_cols=drop_cols)
    # [99]	validation_0-rmse:0.79394	validation_1-rmse:1.00759

    matrix.to_parquet('../data_tmp/features_20209716_0.snappy')
    matrix = pd.read_parquet('../data_tmp/features_20209716_0.snappy')

    # score_drop.sort_values('mean_drop', ascending=False).head(10)
    #                                mean_drop    tstat_drop
    # item_cnt_month_lag_1            0.155495  1.554948e+09
    # date_item_avg_item_lag_1        0.141731  1.417307e+09
    # date_cat_shop_avg_item_lag_1    0.099592  9.959158e+08
    # date_item_avg_item_lag_2        0.057682  5.768188e+08
    # date_cat_shop_avg_item_lag_12   0.053340  5.334039e+08
    # date_item_avg_item_lag_3        0.039735  3.973522e+08
    # date_item_avg_item_lag_12       0.038037  3.803656e+08
    # date_cat_shop_avg_item_lag_6    0.037532  3.753234e+08
    # date_cat_shop_avg_item_lag_2    0.034017  3.401722e+08
    # date_item_avg_item_lag_6        0.031938  3.193794e+08

    drop_cols = ['ID', 'shop_0', 'shop_1', 'shop_2', 'cat_0', 'cat_1', 'cat_2', 'item_0', 'item_1', 'item_2', 'item_3', 'item_4']
    model, X_train, Y_train, X_valid, Y_valid, X_test = train_xgb(matrix, drop_cols=drop_cols, max_depth=8, n_estimators=500, eta=0.02)
    ...
    # xgb_submission_2020-07-16_0.csv
    # valid=0.897063
    # test = 0.93606
...
def features_20209716_1():
    ...
    # drop_cols = ['ID', 'shop_0', 'shop_1', 'shop_2', 'cat_0', 'cat_1', 'cat_2', 'item_0', 'item_1', 'item_2', 'item_3', 'item_4']
    # fs = evaluate_features(matrix, t_start=11, t_valid=[22], drop_cols=drop_cols)
    # [99]	validation_0-rmse:0.78096	validation_1-rmse:0.98112

    ################################################
    # price features
    ################################################
    group = train.groupby(['date_block_num', 'item_id', 'shop_id']).agg({'item_price': ['mean']})
    group.columns = ['date_item_shop_price']
    group.reset_index(inplace=True)
    matrix = matrix.merge(group, on=['date_block_num', 'item_id', 'shop_id'], how='left')
    matrix['date_item_shop_price'] = matrix['date_item_shop_price'].astype(np.float32)
    
    group = train.groupby(['date_block_num', 'item_id']).agg({'item_price': ['mean']})
    group.columns = ['date_item_price']
    group.reset_index(inplace=True)
    matrix = matrix.merge(group, on=['date_block_num', 'item_id'], how='left')
    matrix['date_item_price'] = matrix['date_item_price'].astype(np.float32)
    ...
    group = train.groupby(['item_id', 'shop_id']).agg({'item_price': ['mean']})
    group.columns = ['item_shop_price']
    group.reset_index(inplace=True)
    matrix = matrix.merge(group, on=['item_id', 'shop_id'], how='left')
    matrix['item_shop_price'] = matrix['item_shop_price'].astype(np.float32)

    group = train.merge(items[['item_id', 'item_category_id']], on=['item_id'], how='left').\
        groupby(['item_category_id', 'shop_id']).agg({'item_price': ['mean']})
    group.columns = ['cat_shop_price']
    group.reset_index(inplace=True)
    matrix = matrix.merge(group, on=['item_category_id', 'shop_id'], how='left')
    matrix['cat_shop_price'] = matrix['cat_shop_price'].astype(np.float32)

    group = train.merge(items[['item_id', 'item_category_id']], on=['item_id'], how='left').groupby(['item_category_id']).agg({'item_price': ['mean']})
    group.columns = ['cat_price']
    group.reset_index(inplace=True)
    matrix = matrix.merge(group, on=['item_category_id'], how='left')
    matrix['cat_price'] = matrix['cat_price'].astype(np.float32)

    # filling na's for item price
    matrix['cat_shop_price']      .fillna(matrix['cat_price'], inplace=True)
    matrix['item_price']          .fillna(matrix['cat_shop_price'], inplace=True)
    matrix['date_item_price']     .fillna(matrix['item_price'], inplace=True)
    matrix['date_item_shop_price'].fillna(matrix['date_item_price'], inplace=True)
    matrix['item_shop_price']     .fillna(matrix['item_price'], inplace=True)

    ts = time.time()
    lags = [1, 2, 3, 4, 5, 6]
    matrix = utils.lag_feature(matrix, lags, 'date_item_price')

    ts = time.time()
    lags = [1, 2, 3, 4, 5, 6]
    matrix = utils.lag_feature(matrix, lags, 'date_item_shop_price')

    for i in tqdm(lags):
        matrix['delta_price_lag_' + str(i)] = \
            matrix['date_item_price_lag_' + str(i)] / matrix['item_price'] - 1.0
        matrix['delta_shop_price_lag_' + str(i)] = \
            matrix['date_item_shop_price_lag_' + str(i)] / matrix['item_shop_price'] - 1.0
    gc.collect()
    ...
    matrix.rename({'date_item_shop_price': 'price'}, inplace=True, axis=1)

    fetures_to_drop = ['date_item_price', 'item_price', 'item_shop_price', 'cat_shop_price', 'cat_price']
    for i in lags:
        fetures_to_drop += ['date_item_price_lag_' + str(i)]
        fetures_to_drop += ['delta_price_lag_' + str(i)]
        fetures_to_drop += ['date_item_shop_price_lag_' + str(i)]
        fetures_to_drop += ['delta_shop_price_lag_' + str(i)]
    matrix.drop(fetures_to_drop, axis=1, inplace=True)
    time.time() - ts
    
    matrix.to_parquet('../data_tmp/features_20209716_1.snappy')
    matrix = pd.read_parquet('../data_tmp/features_20209716_1.snappy')

    drop_cols = ['price', 'ID', 'cat_0', 'cat_1', 'cat_2', 'shop_0', 'shop_1', 'shop_2', 'item_0', 'item_1', 'item_2', 'item_3', 'item_4']
    fs = evaluate_features(matrix, t_start=11, t_valid=[22], drop_cols=drop_cols)
    # [99]	validation_0-rmse:0.77548	validation_1-rmse:0.97976
...
def features_20209716_2():
    ...
    group = train.groupby(['date_block_num', 'item_id', 'shop_id']).agg({'revenue': ['sum']})
    group.columns = ['date_item_shop_revenue']
    group.reset_index(inplace=True)
    matrix = matrix.merge(group, on=['date_block_num', 'item_id', 'shop_id'], how='left')
    matrix['date_item_shop_revenue'].fillna(0., inplace=True)

    group = train.merge(items[['item_id', 'item_category_id']], on='item_id', how='left').\
        groupby(['date_block_num', 'item_category_id', 'shop_id']).agg({'revenue': ['sum']})
    group.columns = ['date_cat_shop_revenue']
    group.reset_index(inplace=True)
    matrix = matrix.merge(group, on=['date_block_num', 'item_category_id', 'shop_id'], how='left')
    matrix['date_cat_shop_revenue'].fillna(0., inplace=True)
    ...
    group = train.groupby(['date_block_num', 'shop_id']).agg({'revenue': ['sum']}).\
            reset_index().groupby('shop_id').agg({('revenue', 'sum'): ['mean']})
    group.columns = ['shop_avg_revenue']
    group.reset_index(inplace=True)
    ...
    group = train.merge(items[['item_id', 'item_category_id']], on='item_id', how='left').\
            groupby(['date_block_num', 'item_category_id', 'shop_id']).agg({'revenue': ['sum']}).\
            reset_index().groupby(['item_category_id', 'shop_id']).agg({('revenue', 'sum'): ['mean']})
    group.columns = ['cat_shop_avg_revenue']
    group.reset_index(inplace=True)
    matrix = matrix.merge(group, on=['item_category_id', 'shop_id'], how='left')
    matrix['cat_shop_avg_revenue'].fillna(0., inplace=True)

    matrix['delta_revenue'] = (matrix['date_shop_revenue'] - matrix['shop_avg_revenue']) / matrix['shop_avg_revenue']
    matrix['delta_revenue'] = matrix['delta_revenue'].astype(np.float32)
    matrix['delta_cat_revenue'] = (matrix['date_cat_shop_revenue'] - matrix['cat_shop_avg_revenue']) / (1e-10 + matrix['cat_shop_avg_revenue'])
    matrix['delta_cat_revenue'] = matrix['delta_cat_revenue'].astype(np.float32)

    matrix = utils.lag_feature(matrix, [1], 'delta_revenue')
    matrix = utils.lag_feature(matrix, [1], 'delta_cat_revenue')
    matrix = utils.lag_feature(matrix, [1], 'date_item_shop_revenue')
    matrix = utils.lag_feature(matrix, [1], 'date_cat_shop_revenue')
    matrix['delta_revenue_lag_1'].fillna(0.0, inplace=True)
    matrix['date_cat_shop_revenue_lag_1'].fillna(0.0, inplace=True)
    for c in matrix.columns:
        if matrix[c].dtype == 'float64':
            matrix[c] = matrix[c].astype(np.float32)
    matrix.drop(['delta_cat_revenue', 'cat_shop_avg_revenue', 'date_cat_shop_revenue', 'date_item_shop_revenue', 'date_shop_revenue', 'shop_avg_revenue', 'delta_revenue'], axis=1, inplace=True)

    drop_cols = ['price', 'ID', 'shop_0', 'shop_1', 'shop_2', 'cat_0', 'cat_1', 'cat_2', 'item_0', 'item_1', 'item_2', 'item_3', 'item_4']
    fs = evaluate_features(matrix, t_start=11, t_valid=[22], drop_cols=drop_cols)
    fs.score_drop
    # [99]	validation_0-rmse:0.77388	validation_1-rmse:0.97838

    matrix.to_parquet('../data_tmp/features_20209716_2.snappy')
    matrix = pd.read_parquet('../data_tmp/features_20209716_2.snappy')


def features_20209716_3():
    # train, test, items, shops, cats = load_and_clean()
    matrix = pd.read_parquet('../data_tmp/features_20209716_2.snappy')
    ...
    fs = evaluate_features(matrix, t_start=11, t_valid=[22], drop_cols=drop_cols)
    fs.score_drop
    # [99]	validation_0-rmse:0.77076	validation_1-rmse:0.96003
    ...
    for agg_cols, agg_names, lags in tqdm(
            [(['date_block_num'], ['date'], [1]),
             ...
             (['date_block_num', 'cat_1'], ['date', 'cat_1'], [1]),
             (['date_block_num', 'cat_2'], ['date', 'cat_2'], [1]),
             (['date_block_num', 'shop_id'], ['date', 'cat_0'], [1]),
             (['date_block_num', 'shop_id'], ['date', 'cat_1'], [1]),
             (['date_block_num', 'shop_id'], ['date', 'cat_2'], [1]),
             (['date_block_num', 'item_id'], ['date', 'shop_0'], [1]),
             (['date_block_num', 'item_id'], ['date', 'shop_1'], [1]),
             (['date_block_num', 'item_id'], ['date', 'shop_2'], [1]),]):
        ts = time.time()
        mean_enc_name = '_'.join(agg_names + ['avg', 'item_cnt'])
        group = matrix.groupby(agg_cols).agg({'item_cnt_month': ['mean']})
        group.columns = [mean_enc_name]
    ...
    # drop_cols = ['price', 'ID', 'shop_0', 'shop_1', 'shop_2', 'cat_0', 'cat_1', 'cat_2', 'item_0', 'item_1', 'item_2', 'item_3', 'item_4']
    drop_cols = ['price', 'ID', ]
    drop_cols += ['date_cat_avg_item_lag_1', 'date_cat_avg_item_lag_2', 'date_cat_avg_item_lag_3', 'date_cat_avg_item_lag_6', 'date_cat_avg_item_lag_12', ]
    drop_cols += ['date_cat_1_avg_item_cnt_lag_1_y', 'date_cat_2_avg_item_cnt_lag_1_y', 'date_item_avg_item_lag_2', 'date_item_avg_item_lag_3', 'date_item_avg_item_lag_6', ]
    # drop_cols += ['date_shop_1_avg_item_cnt_lag_1_y', 'date_item_avg_item_lag_3', 'date_shop_2_avg_item_cnt_lag_1_y', 'date_cat_shop_revenue_lag_1', 'date_shop_0_avg_item_cnt_lag_1_x', 'date_shop_1_avg_item_cnt_lag_1_x', ]
    fs = evaluate_features(matrix, t_start=11, t_valid=[22], drop_cols=drop_cols)
    fs.score_drop
    # [99]	validation_0-rmse:0.74249	validation_1-rmse:0.92382
    # [99]	validation_0-rmse:0.74520	validation_1-rmse:0.91885
    # [99]	validation_0-rmse:0.74635	validation_1-rmse:0.91841
    model, X_train, Y_train, X_valid, Y_valid, X_test = train_xgb(matrix, drop_cols=drop_cols, max_depth=8, n_estimators=500, eta=0.02)
    # [246]	validation_0-rmse:0.78183	validation_1-rmse:0.86930
    # [201]	validation_0-rmse:0.78847	validation_1-rmse:0.87016
    submission_name = gen_submission(model, X_train, Y_train, X_valid, Y_valid, X_test)
    # train-rmse=0.6113 valid-rmse=0.7557
    # train-rmse=0.6217 valid-rmse=0.7572
    importances = pd.Series({f: v for f, v in zip(X_train.columns, model.feature_importances_)}).sort_values()
    score_drop = feature_stats.calc_score_drop(None, model, importances, X_valid, Y_valid, clip_down=0, clip_up=20)
    submission_message = 'xgb score_drop best 10: %s\nntree=%s valid=%s' % (', '.join(score_drop.index[::-1][:10]), model.best_iteration, model.best_score)
    print(submission_message)
    print(submission_name)
    # xgb_submission_2020-07-17_0.csv
    # valid=0.869304
    # test=0.89803
    # xgb_submission_2020-07-17_2.csv
    # valid=0.87016
    # test=0.89897
    for c in matrix.columns:
        if matrix[c].dtype == 'float16':
            matrix[c] = matrix[c].astype(np.float32)
    matrix.to_parquet('../data_tmp/features_20209716_3.snappy')


def features_20209716_4():
    # train, test, items, shops, cats = load_and_clean()
    matrix = pd.read_parquet('../data_tmp/features_20209716_3.snappy')

    drop_cols = ['date_avg_item_cnt_lag_1', 'date_shop_0_avg_item_cnt_lag_1_x',
       'date_shop_1_avg_item_cnt_lag_1_x', 'date_shop_2_avg_item_cnt_lag_1_x',
       'date_cat_0_avg_item_cnt_lag_1_x', 'date_cat_1_avg_item_cnt_lag_1_x',
       'date_cat_2_avg_item_cnt_lag_1_x', 'date_cat_0_avg_item_cnt_lag_1_y',
       'date_cat_1_avg_item_cnt_lag_1_y', 'date_cat_2_avg_item_cnt_lag_1_y',
       'date_shop_0_avg_item_cnt_lag_1_y', 'date_shop_1_avg_item_cnt_lag_1_y',
       'date_shop_2_avg_item_cnt_lag_1_y']
    matrix.drop(drop_cols, axis=1, inplace=True)

    matrix.to_parquet('../data_tmp/features_20209716_4.snappy')

    drop_cols = ['price', 'ID', ]
    fs = evaluate_features(matrix, t_start=11, t_valid=[22], drop_cols=drop_cols)
    fs.score_drop
    # [99]	validation_0-rmse:0.74325	validation_1-rmse:0.92466
    model, X_train, Y_train, X_valid, Y_valid, X_test = train_xgb(matrix, drop_cols=drop_cols, max_depth=8, n_estimators=500, eta=0.02)
    submission_name = gen_submission(model, X_train, Y_train, X_valid, Y_valid, X_test)
    # train-rmse=0.6044 valid-rmse=0.7461
    importances = pd.Series({f: v for f, v in zip(X_train.columns, model.feature_importances_)}).sort_values()
    score_drop = feature_stats.calc_score_drop(None, model, importances, X_valid, Y_valid, clip_down=0, clip_up=20)
    submission_message = 'xgb score_drop best 10: %s\nntree=%s valid=%s' % (', '.join(score_drop.index[::-1][:10]), model.best_iteration, model.best_score)
    print(submission_message)
    ...
    # valid=0.87016
    # test=0.89897
    # xgb_submission_2020-07-17_6.csv
    # valid=.86377
    # test=0.89450
...
#                       subsample=0.8,
    ...
    def find_best():
        ...
        def train_xgb_wrapper(param):
            param['max_depth'] = int(param['max_depth'])
            print(param)
            model, X_train, Y_train, X_valid, Y_valid, X_test = train_xgb(matrix, path=None,
                          target='item_cnt_month',
                          drop_cols=['ID', ],
                          # drop_cols=['price', 'ID', ],
                          add_drop=['date_block_num',],
                          t_start=11,
                          t_valid=[33], **param)
        ...
        space = {
            'eta': 0.05,
            'max_depth': hp.quniform('max_depth', 8, 12, 2),
            'min_child_weight': hp.quniform('min_child_weight', 100, 600, 100),
        ...
        # {'colsample_bytree': 0.6, 'eta': 0.05, 'max_depth': 12, 'min_child_weight': 400.0, 'n_estimators': 300} 0.85730
    ...
    model, X_train, Y_train, X_valid, Y_valid, X_test = train_xgb(matrix, drop_cols=drop_cols, n_estimators=500, eta=0.02,
                                                                  colsample_bytree=0.6, max_depth=10, min_child_weight=500)
    submission_name = gen_submission(model, X_train, Y_train, X_valid, Y_valid, X_test)
    # train-rmse=0.5820 valid-rmse=0.7189
    importances = pd.Series({f: v for f, v in zip(X_train.columns, model.feature_importances_)}).sort_values()
    score_drop = feature_stats.calc_score_drop(None, model, importances, X_valid, Y_valid, clip_down=0, clip_up=20)
    submission_message = 'xgb score_drop best 10: %s\nntree=%s valid=%s' % (', '.join(score_drop.index[::-1][:10]), model.best_iteration, model.best_score)
    print(submission_message)
    ...
    # valid=.86377
    # test=0.89450
    # xgb_submission_2020-07-17_10.csv
    # valid=0.85921
    # test=0.89355
    # xgb_submission_2020-07-30_10.csv
    # valid=0.847916
    # test=0.91932
    ...
    # valid=0.88035
    # test=0.88250
    # xgb_submission_2020-07-20_0.csv
    # valid=0.876039
    # test=0.86914

    matrix = pd.read_parquet('../data_tmp/data_2020-06-14_01_replica.snappy')
    drop_cols = []
    drop_cols += ['date_item_avg_item_cnt_lag_2', 'date_item_avg_item_cnt_lag_3',
...
def features_20200614_5():
    train, test, items, shops, cats = load_and_clean(1)
    # matrix = extend_train(train, test, items, shops, cats)
    matrix = pd.read_parquet('../data_tmp/extend_train.snappy')
    ...
    for agg_cols, agg_names, lags in tqdm(
            [(['date_block_num', ], ['date', ], [1]),
             ...
             (['date_block_num', 'shop_id'], ['date', 'shop'], [1, 2, 3, 6, 12]),
             (['date_block_num', 'item_category_id'], ['date', 'cat'], [1, 2, 3, 6, 12]),
             (['date_block_num', 'shop_id', 'item_category_id'], ['date', 'shop', 'cat'], [1, 2, 3, 6, 12]),
             (['date_block_num', 'shop_0'], ['date', 'shop_0'], [1]),
             (['date_block_num', 'shop_1'], ['date', 'shop_1'], [1]),
             (['date_block_num', 'shop_2'], ['date', 'shop_2'], [1]),
             (['date_block_num', 'cat_0'], ['date', 'cat_0'], [1]),
    ...
    matrix.to_parquet('../data_tmp/data_2020-06-14_02.snappy')

    # keep_cols = ['ID', 'item_id', 'item_cnt_month', 'date_block_num']
    # keep_cols += ['cat_0', 'cat_1', 'cat_2', 'date_avg_item_cnt_lag_1', 'date_cat_0_avg_item_cnt_lag_1', 'date_cat_1_avg_item_cnt_lag_1', 'date_cat_2_avg_item_cnt_lag_1', 'date_cat_avg_item_cnt_lag_1', 'date_item_avg_item_cnt_lag_1', 'date_item_avg_item_cnt_lag_6', 'date_shop_0_avg_item_cnt_lag_1', 'date_shop_1_avg_item_cnt_lag_1', 'date_shop_avg_item_cnt_lag_1', 'date_shop_avg_item_cnt_lag_2', 'date_shop_avg_item_cnt_lag_3', 'date_shop_avg_item_cnt_lag_6', 'date_shop_avg_item_cnt_lag_12', 'date_shop_cat_avg_item_cnt_lag_1', 'delta_price_lag', 'delta_revenue_lag_1', 'item_0', 'item_1', 'item_2', 'item_3', 'item_4', 'item_category_id', 'item_cnt_month_lag_1', 'item_cnt_month_lag_2', 'item_cnt_month_lag_3', 'item_cnt_month_lag_6', 'item_cnt_month_lag_12', 'item_first_sale', 'item_shop_first_sale', 'shop_0', 'shop_1', 'shop_2', 'shop_id', ]
    # matrix = pd.read_parquet('../data_tmp/data_2020-06-14_01_.snappy')[keep_cols]
    # matrix = utils.reduce_mem_usage(matrix)
    # drop_cols = []
    # model, X_train, Y_train, X_valid, Y_valid, X_test = train_xgb(matrix, drop_cols=drop_cols, n_estimators=1000, eta=0.02,
    #                                                               colsample_bytree=0.6, max_depth=12, min_child_weight=200,
    #                                                               early_stopping_rounds=10)
    # submission_name = gen_submission(model, X_train, Y_train, X_valid, Y_valid, X_test)
    # importances = pd.Series({f: v for f, v in zip(X_train.columns, model.feature_importances_)}).sort_values()
    ...
    # score_drop[score_drop.mean_drop <= 0]
    # # same_cols, clusters = utils.find_duplicates_features(matrix, corr_sample_size=100000)
    # # score_drop.to_csv('../data_tmp/score_drop_159_subission2.csv')
    # # importances.to_csv('../data_tmp/importances_159_subission2.csv')
    # submission_message = 'xgb score_drop best 10: %s\nntree=%s valid=%s' % (', '.join(score_drop.index[::-1][:10]), model.best_iteration, model.best_score)
    # print(submission_message)
    # print(submission_name)
    # # xgb_submission_2020-07-17_0.csv
...
def features_20200614_5():
    matrix = pd.read_parquet('../data_tmp/data_2020-06-14_02.snappy')
    n_round = 2
    for c in ['item_%s' % i for i in range(5)]:
        matrix[c] = matrix[c].round(n_round)
        matrix[c] = (matrix[c] - matrix[c].min()) / (matrix[c].max() - matrix[c].min() + 1e-10)
    for c in ['shop_%s' % i for i in range(3)]:
        matrix[c] = matrix[c].round(n_round)
        matrix[c] = (matrix[c] - matrix[c].min()) / (matrix[c].max() - matrix[c].min() + 1e-10)
    for c in ['cat_%s' % i for i in range(3)]:
        matrix[c] = matrix[c].round(n_round)
        matrix[c] = (matrix[c] - matrix[c].min()) / (matrix[c].max() - matrix[c].min() + 1e-10)

    t_start = 11
    target = 'item_cnt_month'
    all_items = set(['item_%s' % i for i in range(5)])
    for c in tqdm(['item_%s' % i for i in range(5)]):
        # cs = sorted(all_items - set([c]))
        # group = matrix[matrix.date_block_num <= t_start].groupby(cs)[target].agg(['mean'])
        group = matrix[matrix.date_block_num <= t_start].groupby(c)[target].agg(['mean'])
        group.columns = ['%s_avg_item' % c]
        group.reset_index(inplace=True)
        matrix = matrix.merge(group, on=c, how='left')

    t_start = 11
    target = 'item_cnt_month'
    for c in tqdm(['shop_%s' % i for i in range(3)]):
        group = matrix[matrix.date_block_num <= t_start].groupby(c)[target].agg(['mean'])
        group.columns = ['%s_avg_item' % c]
        group.reset_index(inplace=True)
        matrix = matrix.merge(group, on=c, how='left')
    ...
    t_start = 11
    target = 'item_cnt_month'
    for c in tqdm(['cat_%s' % i for i in range(3)]):
        group = matrix[matrix.date_block_num <= t_start].groupby(c)[target].agg(['mean'])
        group.columns = ['%s_avg_item' % c]
        group.reset_index(inplace=True)
        matrix = matrix.merge(group, on=c, how='left')

    group = matrix[matrix.date_block_num <= t_start].groupby('shop_id')[target].agg(['mean'])
    group.columns = ['shop_avg_item']
    group.reset_index(inplace=True)
    matrix = matrix.merge(group, on='shop_id', how='left')

    group = matrix[matrix.date_block_num <= t_start].groupby('item_category_id')[target].agg(['mean'])
    group.columns = ['cat_avg_item']
    group.reset_index(inplace=True)
    matrix = matrix.merge(group, on='item_category_id', how='left')
    ...
    drop_cols += [
                ...
                'date_cat_1_avg_item_cnt_lag_1',
                'date_item_avg_item_cnt_lag_2',
                'date_item_avg_item_cnt_lag_6',
                'date_shop_0_avg_item_cnt_lag_1',
                'date_shop_1_avg_item_cnt_lag_1',
                'date_shop_2_avg_item_cnt_lag_1',
    ...
    def find_best():
        ...
        best = fmin(train_xgb_wrapper, space, algo=tpe.suggest, max_evals=30, trials=trials)
        pd.concat([pd.DataFrame(trials.vals), pd.Series(trials.losses(), name='loss')], axis=1).\
            drop_duplicates().to_csv('../data_tmp/hyperopt1.csv')
    ...
    matrix = pd.read_parquet('../data_tmp/data_2020-06-14_02_.snappy')
    matrix = utils.reduce_mem_usage(matrix)
    model, X_train, Y_train, X_valid, Y_valid, X_test = train_xgb(matrix, drop_cols=drop_cols, n_estimators=1000, eta=0.02,
                                                                  colsample_bytree=0.6, max_depth=12, min_child_weight=100,
