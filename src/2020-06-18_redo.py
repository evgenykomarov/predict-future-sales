...
def rolling_mean_encoding(matrix, target='item_cnt_month', group_cols=[], global_mean=None):
...
matrix.to_parquet('../data_tmp/tmp0618_01.snappy')
matrix = pd.read_parquet('../data_tmp/tmp0618_01.snappy')
...
encoded_feature = rolling_mean_encoding(matrix, 'item_cnt_month', ['item_category_id'])
...
encoded_feature = rolling_mean_encoding(matrix, 'item_cnt_month', ['shop_id'])
matrix[mean_enc_name] = encoded_feature
matrix = lag_feature(matrix, [1, 12], mean_enc_name)
matrix.drop([mean_enc_name], axis=1, inplace=True)
...
encoded_feature = rolling_mean_encoding(matrix, 'item_cnt_month', ['item_category_id', 'shop_id'])
matrix[mean_enc_name] = encoded_feature
matrix = lag_feature(matrix, [1, 12], mean_enc_name)
matrix.drop([mean_enc_name], axis=1, inplace=True)
...
matrix.to_parquet('../data_tmp/tmp0618_02.pkl')
matrix = pd.read_parquet('../data_tmp/tmp0618_02.pkl')
...
for f0, f1 in [('item_cnt_month_lag_1', 'item_cnt_month_lag_2')]:
    ...
    group_cols = ['f0', 'f1']
    encoded_feature = rolling_mean_encoding(matrix, 'item_cnt_month', group_cols)
...
matrix.to_parquet('../data_tmp/tmp0618_03.pkl')
matrix = pd.read_parquet('../data_tmp/tmp0618_03.pkl')

mean_enc_name = '_'.join(['item', 'avg', 'item'])
encoded_feature = rolling_mean_encoding(matrix, 'item_cnt_month', ['item_id'])
matrix[mean_enc_name] = encoded_feature
matrix = lag_feature(matrix, [1, 12], mean_enc_name)
matrix.drop([mean_enc_name], axis=1, inplace=True)
...
matrix.to_parquet('../data_tmp/tmp0618_04.pkl')
matrix = pd.read_parquet('../data_tmp/tmp0618_04.pkl')
...
matrix.to_parquet('../data_tmp/tmp0618_05.pkl')
matrix = pd.read_parquet('../data_tmp/tmp0618_05.pkl')
...
for c in matrix.columns:
    if matrix[c].dtype == 'float16' or matrix[c].dtype == 'float64':
        matrix[c] = matrix[c].astype(np.float32)
matrix.to_parquet('../data_tmp/tmp0618_06.pkl')
matrix = pd.read_parquet('../data_tmp/tmp0618_06.pkl')
...
matrix.to_parquet('../data_tmp/tmp0618_07.pkl')
matrix = pd.read_parquet('../data_tmp/tmp0618_07.pkl')
...
matrix.to_parquet('../data_tmp/tmp0618_08.pkl')
matrix = pd.read_parquet('../data_tmp/tmp0618_08.pkl')
...
data = pd.read_parquet('../data_tmp/tmp0618_08.pkl')
sorted(data.columns)
drop_cols = ['ID', ]
drop_cols = ['ID', 'shop_id', 'item_id', 'item_category_id']
...
import pickle
pickle.dump(model, open('../data/model_xgb_2020-06-18_01.pkl', "wb"))
model = pickle.load(open('../data/model_xgb_2020-06-18_01.pkl', "rb"))
...
submission.to_csv('../submission/xgb_submission_2020-07-15_01.csv', index=False)
pd.Series(Y_pred).to_csv('../submission/xgb_submission_2020-07-15_01_valid.csv', index=False)
...
score_drop = pd.Series(score_drop)
score_drop[score_drop < 0]
...
submission.to_csv('../submission/xgb_submission_2020-07-15_02.csv', index=False)
pd.Series(Y_pred).to_csv('../submission/xgb_submission_2020-07-15_02_valid.csv', index=False)
