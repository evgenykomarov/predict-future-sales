...
def load_and_clean(alt_embed=2):
    ...
    if alt_embed == 0 or alt_embed == 1:
        ...
        item_features2 = text_features.text_feature_to_vec2(items.item_name.values, D=6, epochs=50, learning_rate=0.0005)
        text_features.show_neighbors(items.item_name.values, item_features)

        shop_features = text_features.text_feature_to_vec2(shops.shop_name.values, D=3, epochs=500, learning_rate=0.01)
        text_features.show_neighbors(shops.shop_name.values, shop_features)

        cat_features = text_features.text_feature_to_vec2(cats.item_category_name.values, D=3, epochs=500, learning_rate=0.01)
...
def features_20209716_0():
    ...
    # [9]	validation_0-rmse:0.89389	validation_1-rmse:1.07929
    ...
    drop_cols = ['ID', 'shop_0', 'shop_1', 'shop_2', 'cat_0', 'cat_1', 'cat_2', 'item_0', 'item_1', 'item_2', 'item_3', 'item_4']
    fs = evaluate_features(matrix, t_start=11, t_valid=[22], drop_cols=drop_cols)
    # [9]	validation_0-rmse:0.82161	validation_1-rmse:1.00771
...
def features_20209716_2():
    ...
    drop_cols = ['price', 'ID', 'shop_0', 'shop_1', 'shop_2', 'cat_0', 'cat_1', 'cat_2', 'item_0', 'item_1', 'item_2', 'item_3', 'item_4']
    drop_cols += ['delta_cat_revenue', 'cat_shop_avg_revenue', 'date_cat_shop_revenue', 'date_item_shop_revenue', 'date_shop_revenue','shop_avg_revenue','delta_revenue']
    fs = evaluate_features(matrix, t_start=11, t_valid=[22], drop_cols=drop_cols)
    fs.score_drop
    # [9]	validation_0-rmse:0.79891	validation_1-rmse:0.99045
    # [9]	validation_0-rmse:0.80128	validation_1-rmse:0.98295
    # [9]	validation_0-rmse:0.80247	validation_1-rmse:0.98382
...
def features_20209716_3():
    ...
    # # [9]	validation_0-rmse:0.80128	validation_1-rmse:0.98295
    # # [9]	validation_0-rmse:0.80247	validation_1-rmse:0.98382
    # # [9]	validation_0-rmse:0.80034	validation_1-rmse:0.98822
    ...
    fs = evaluate_features(matrix, t_start=11, t_valid=[22], drop_cols=drop_cols)
    fs.score_drop
    # [9]	validation_0-rmse:0.77591	validation_1-rmse:0.95733
    # [9]	validation_0-rmse:0.77816	validation_1-rmse:0.95745
    model, X_train, Y_train, X_valid, Y_valid, X_test = train_xgb(matrix, drop_cols=drop_cols, max_depth=8, n_estimators=500, eta=0.02)
...
def features_20209716_4():
    ...
    fs.score_drop
    # [9]	validation_0-rmse:0.77816	validation_1-rmse:0.95745
    # [9]	validation_0-rmse:0.78158	validation_1-rmse:0.95863
    model, X_train, Y_train, X_valid, Y_valid, X_test = train_xgb(matrix, drop_cols=drop_cols, max_depth=8, n_estimators=500, eta=0.02)
    submission_name = gen_submission(model, X_train, Y_train, X_valid, Y_valid, X_test)
...
#                       subsample=0.8,
    ...
    model, X_train, Y_train, X_valid, Y_valid, X_test = train_xgb(matrix, drop_cols=drop_cols, n_estimators=500, eta=0.02,
                                                                  colsample_bytree=0.6, max_depth=12, min_child_weight=400)
    submission_name = gen_submission(model, X_train, Y_train, X_valid, Y_valid, X_test)
    # train-rmse=0.6044 valid-rmse=0.7461
    importances = pd.Series({f: v for f, v in zip(X_train.columns, model.feature_importances_)}).sort_values()
    score_drop = feature_stats.calc_score_drop(None, model, importances, X_valid, Y_valid, clip_down=0, clip_up=20)
...
def features_20200614_5():
    train, test, items, shops, cats = load_and_clean(True)
    matrix = extend_train(train, test, items, shops, cats)
