...
ts = time.time()
for c in tqdm(['cat_0', 'cat_1', 'cat_2']):
    mean_enc_name = 'date_shop_' + c + '_avg_item_cnt'
    group = matrix.groupby(['date_block_num', 'shop_id', c]).agg({'item_cnt_month': ['mean']})
    group.columns = [mean_enc_name]
    group.reset_index(inplace=True)
    
    matrix = pd.merge(matrix, group, on=['date_block_num', 'shop_id', c], how='left')
    matrix[mean_enc_name] = matrix[mean_enc_name].astype(np.float16)
    matrix = lag_feature(matrix, [1], mean_enc_name)
...
for c in tqdm(['shop_0', 'shop_1', 'shop_2']):
    mean_enc_name = 'date_' + c + '_avg_item_cnt'
    group = matrix.groupby(['date_block_num', c]).agg({'item_cnt_month': ['mean']})
    group.columns = [mean_enc_name]
    group.reset_index(inplace=True)
    
    matrix = pd.merge(matrix, group, on=['date_block_num', c], how='left')
    matrix[mean_enc_name] = matrix[mean_enc_name].astype(np.float16)
    matrix = lag_feature(matrix, [1], mean_enc_name)
    matrix.drop([mean_enc_name], axis=1, inplace=True)
time.time() - ts
...
ts = time.time()
for c in tqdm(['shop_0', 'shop_1', 'shop_2']):
    mean_enc_name = 'date_item_' + c + '_avg_item_cnt'
    group = matrix.groupby(['date_block_num', 'item_id', c]).agg({'item_cnt_month': ['mean']})
    group.columns = [mean_enc_name]
    group.reset_index(inplace=True)
    
    matrix = pd.merge(matrix, group, on=['date_block_num', 'item_id', c], how='left')
    matrix[mean_enc_name] = matrix[mean_enc_name].astype(np.float16)
    matrix = lag_feature(matrix, [1], mean_enc_name)
    matrix.drop([mean_enc_name], axis=1, inplace=True)
time.time() - ts
...
row = matrix.iloc[-500000, :]
bool(row['delta_price_lag_' + str(6)])
row.index
...
matrix['delta_price_lag_1'][matrix['delta_price_lag_1'] > 0].index
matrix.apply(select_trend, axis=1)
if np.NAN:
    print('a')

matrix['delta_price_lag'] = matrix['delta_price_lag_1'] #matrix.apply(select_trend, axis=1)
...
submission.to_csv('../data/xgb_submission2.csv', index=False)
